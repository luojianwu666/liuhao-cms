...
public class HLUtils {
	...
	public static PageInfo<?> findByHighLight(ElasticsearchTemplate elasticsearchTemplate, Class<?> clazz, Integer page,
		...
		pi.setPageNum(page);
		pi.setPageSize(rows);
		pi.setPrePage(page-1<0?page:page-1);
		pi.setNextPage(page-1>0?page+1:pages);
		pi.setLastPage(page+1);
		pi.setPages(pages);
		List content = pageInfo.getContent();
		pi.setList(content);

		int[] num = new int [pi.getPages()];
		for(int i=0;i<pi.getPages();i++) {
			num[i]=i+1;
		}
		pi.setNavigatepageNums(num);
